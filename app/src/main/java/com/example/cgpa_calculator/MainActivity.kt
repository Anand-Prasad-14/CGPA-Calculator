        ...
        Row() {
            Column() {
                Button(
                    onClick = {
                        ...
                        if (totalCredit > 0) {
                            cgpa = totalGrde / totalCredit.toDouble()
                        } else {
                            cgpa = 0.0
                        }
                        grade1 = ""
            ...
            Surface(
                ...
                Column {
                    Text(
                        modifier = Modifier.fillMaxWidth(),
                        text = "Previus Semester", style = TextStyle(
                            fontFamily = FontFamily.Default,
                            fontSize = 16.sp,
                            color = Color(0xFFFFFFFF),
                            textAlign = TextAlign.Center
                    ...
                    if (semesters.isNotEmpty()) {
                        for (semester in semesters) {
                            Text(
                                text = "Garde: ${semester.grade} , Credit: ${semester.credit}",
                                color = Color.White,
                                fontSize = 16.sp,
                                modifier = Modifier.fillMaxWidth(),
                                textAlign = TextAlign.Center
...
fun GradeTextField(grade: String, onValueChange: (String) -> Unit) {
    TextField(
        value = grade, onValueChange = { text -> onValueChange(text) }, modifier = Modifier
            .fillMaxWidth()

            .height(47.dp), label = {
            Text(
                text = "Enter Grade", color = Color.White, fontSize = 12.sp
            )
...
fun CreditTextField(credit: Int?, onValueChange: (Int?) -> Unit) {
    TextField(
        ...
        label = {
            Text(
                text = "Enter Credit", color = Color.Black, fontSize = 12.sp
            )
        },
        colors = TextFieldDefaults.textFieldColors(
            focusedIndicatorColor = Color.Transparent,
